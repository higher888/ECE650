...
//		printf("%d %d\n", k, VerticeNum * k);
		...
		/*fourth clause type*/
		for(i=0;i<h.numVertexes;i++) 
		{
			for(j=i+1;j<h.numVertexes;j++)
			{	
				if (h.arc[i][j]==1)
				{
					for(o = 1;o <= k; o++)
	...
	printf("Vertex No. of CNF-SAT-VC is: %d\n",vNum);
	return NULL;
}
...
void* VerCover_APPROX1(void* arg) 
{
	...
	printf("Vertex No. of APPROX-VC-1 is: %d\n",vNum);
...
void* VerCover_APPROX2(void* arg) 
{
	...
	printf("Vertex No. of APPROX-VC-2 is: %d\n",vNum);
