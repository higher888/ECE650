...
{
	Graph_struct* h = (Graph_struct*) arg; 

	int VerticeNum;
	int vNum;
	MGraph G;
	VerticeNum = h -> VertexNum;
	G = h -> G;
		...
		for(i=0;i<VerticeNum-1;i++)
		{
			for(j=i+1;j<VerticeNum;j++)
			{
				if (a[i][j]==1)
				{
					...
					EdgeNum = EdgeNum - 1; //an edge <i,j> is used
					for(m = 0;m<VerticeNum;m++)
					{
						if(a[i][m] == 1)
						{
							a[i][m] = 0;
...
int main(int argc, char** argv)
{
	...
	/*if (str[0] != 's')
	   ...
	   //printf("%d \n",VertexNum);
	    ...
	    {
	    	//struct Graph_struct thread_args;
		Graph_struct* g = (Graph_struct*)malloc(sizeof(Graph_struct)*3);
		for(i=0;i<3;i++) 
		{
			g[i].G = G;  //possible error
			g[i].VertexNum = VertexNum;
			g[i].EdgeNum = Edge_num;
		}
		//printf("test\n");
		//fflush(stdout);
		s = pthread_create (&g[0].thread,NULL,&VerCover_CNFSAT,&g[0]);
		if (s != 0)
               		handle_error_en(s, "pthread_create");
		s = pthread_getcpuclockid(g[0].thread, &cid_CNFSAT);
		if (s != 0)
               		handle_error_en(s, "pthread_getcpuclockid");
		pthread_join(g[0].thread,NULL);
		char ms1[] = "CNF-SAT-VC's CPU time:  ";
		pclock(ms1, cid_CNFSAT);
		sleep(1);
		
		s = pthread_create (&g[1].thread,NULL,&VerCover_APPROX1,&g[1]);
		if (s != 0)
               		handle_error_en(s, "pthread_create");
		s = pthread_getcpuclockid(g[1].thread, &cid_APPROX1);
		if (s != 0)
               		handle_error_en(s, "pthread_getcpuclockid"); 
		pthread_join(g[1].thread,NULL);
		char ms2[] = "APPROX-VC-1's CPU time:  ";
		pclock(ms2, cid_APPROX1);
		sleep(1);

		s = pthread_create (&g[2].thread,NULL,&VerCover_APPROX2,&g[2]);
		if (s != 0)
               		handle_error_en(s, "pthread_create");
		s = pthread_getcpuclockid(g[2].thread, &cid_APPROX2);
		if (s != 0)
               		handle_error_en(s, "pthread_getcpuclockid"); 
		char ms3[] = "APPROX-VC-2's CPU time:  ";		
		pthread_join(g[2].thread,NULL);	
